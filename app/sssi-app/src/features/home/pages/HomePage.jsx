    ...
    return (
        <Card
            ...
            sx={(theme) => ({
                ...
                position: 'relative',
                overflow: 'hidden',
                borderColor: `color-mix(in srgb, ${theme.vars.palette.primary.main} 20%, transparent)`,
                background: `linear-gradient(180deg, color-mix(in srgb, ${theme.vars.palette.background.paper} 92%, transparent) 0%, color-mix(in srgb, ${theme.vars.palette.background.paperWarm} 95%, transparent) 100%)`,
            })}
        >
            <CardContent>
                <Stack direction="row" alignItems="flex-start" justifyContent="space-between" gap={2}>
                    <Box>
                        <Typography sx={{ fontSize: 12, color: 'text.secondary', mb: 0.75, textTransform: 'uppercase', letterSpacing: '0.08em' }}>
                            {title}
                        </Typography>
                        <Typography sx={{ fontSize: { xs: 24, md: 28 }, fontWeight: 900, lineHeight: 1.1 }}>{value}</Typography>
                        <Typography sx={{ fontSize: 12, color: 'text.disabled', mt: 0.75 }}>{caption}</Typography>
                    </Box>
                    <Box
                        sx={(theme) => ({
                            width: 38,
